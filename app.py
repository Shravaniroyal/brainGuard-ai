...
def validate_brain_mri(file_bytes):
    ...
    Uses 6 checks to reject non-MRI images.
    """
    try:
        ...
        # CHECK 1: Color — MRI scans are grayscale
        color_diff = np.mean(np.abs(r - g) + np.abs(r - b) + np.abs(g - b)) / 3
        if color_diff > 18:
            return False, "❌ Rejected — colorful photo detected, not a brain MRI."

        # CHECK 2: Mean brightness — MRI scans are mostly dark (mean 10–100)
        # Screenshots, photos, UI images are bright (mean > 100)
        mean_intensity = np.mean(gray)
        if mean_intensity > 100:
            return False, "❌ Rejected — image is too bright. Brain MRI scans have dark backgrounds (mostly black)."

        if mean_intensity < 5:
            return False, "❌ Rejected — image is completely black."

        # CHECK 3: Dark pixel ratio — MRI background is >50% black pixels
        dark_pixels = np.sum(gray < 30) / gray.size
        if dark_pixels < 0.45:
            return False, "❌ Rejected — insufficient dark background. Brain MRIs are mostly black."

        # CHECK 4: Texture variance — MRI has smooth gradients (not sharp text edges)
        variance = np.var(gray)
        if variance < 200:
            return False, "❌ Rejected — no medical imaging texture detected."

        # CHECK 5: Bright region check — MRI has limited bright tissue (10–35% of image)
        bright_pixels = np.sum(gray > 80) / gray.size
        if bright_pixels > 0.40:
            return False, "❌ Rejected — too many bright pixels. Not a brain MRI scan."

        # CHECK 6: Aspect ratio — MRI scans are roughly square or portrait (not ultrawide)
        h, w = gray.shape
        aspect = w / h
        if aspect > 3.0 or aspect < 0.3:
            return False, "❌ Rejected — unusual image dimensions for a brain MRI."

        return True, "✅ Valid brain MRI detected"

    except Exception as e:
        return False, "❌ Could not read file. Please upload a JPG, PNG, or NIfTI brain MRI scan."
